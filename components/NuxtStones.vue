...
const littleStonesTexture = await useTexture([
  "/nuxt-stones/LittleRocksBaked.png",
]);
...
const littleStones = Object.values(nodes).filter((node) =>
  node.name.includes("Stone00")
);
...
<template>
  ...
  <primitive v-for="stone in littleStones" :key="stone.id" :object="stone" />
</template>
