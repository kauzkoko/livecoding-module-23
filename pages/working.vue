<template>
  <div>
    <div id="p5" />
    <div ref="drag1" id="drag-1" class="draggable">1</div>
...
const newPosition = {
  x: attractiveBodiesData[0].position.x,
  y: attractiveBodiesData[0].position.y,
};
const drag1 = ref();
onMounted(() => {
  drag1.value.style.transform = `translate(${newPosition.x - 25}px, ${
    newPosition.y - 25
  }px)`;
  drag1.value.setAttribute("data-x", newPosition.x - 25);
  drag1.value.setAttribute("data-y", newPosition.y - 25);
  ...
  Events.on(engine, "afterUpdate", function () {
    ...
    Body.translate(bodies[0], {
      x: (newPosition.x - bodies[0].position.x + 25) * 0.25,
      y: (newPosition.y - bodies[0].position.y + 25) * 0.25,
    });
  });
